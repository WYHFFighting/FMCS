...
def evaluate(embeds, idx_train, idx_val, idx_test, labels, task,epoch,lr ,isTest=True, iterater=10, args = None):
    ...
    if task == 'Node':
        if isTest:
            ...
            with open('result.txt', 'a') as fa:
                if args is not None:
                    fa.write('gdc alpha: {}\n'.format(args.gdc_alp))
                fa.write("[Classification] Macro-F1: {:.4f} ({:.4f}) | Micro-F1: {:.4f} ({:.4f})\n".format(
                    np.mean(macro_f1s), np.std(macro_f1s), np.mean(micro_f1s), np.std(micro_f1s)))
