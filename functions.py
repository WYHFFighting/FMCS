...
def trainmultiplex(model, mea_func, U, features, adj_list, negation_adj, args,  optimizer, device, epoch):
    ...
    NodeNeighborLoss, link_loss, match_err = loss_matching_recons(common, private, neighbor, features, U, args, epoch, adj_list, negation_adj)  # 秒算！
    ...
    # loss = total_loss + args.beta * corr + args.lammbda * loss_con  # 16.91s 原先 corr loss 用的是减号
    # 去掉 corr 和 loss_contrastive
    # loss = total_loss

    loss = NodeNeighborLoss + args.alpha * match_err + args.beta * corr + args.gamma * loss_contra
    # loss = total_loss - args.beta * corr  # 梯度反转
    # 废弃 loss = match_err + args.alpha*(recons_err+recons_nei) - args.beta* corr + args.lammbda * loss_con
    # 对 loss 进行消融
    # loss = total_loss 16.95s
...
def loss_matching_recons(common, private, neighbor, features, U, args, epoch, adj_list, negation_adj):  # common, recons, features, U, idx_p_list, args, epoch
    ...
    # total_loss = args.nei_coef * NodeNeighborLoss + args.alpha * link_loss + match_err - pri_match_err
    # total_loss = args.nei_coef * NodeNeighborLoss + args.alpha * link_loss + match_err
    # total_loss += args.alpha * link_loss
    # 对 loss 进行消融
    # total_loss = NodeNeighborLoss + match_err  # 16.94s
    # total_loss = args.alpha * link_loss + match_err  # 0.004983s
    # 主要是 NodeNeighborLoss 耗时过大
